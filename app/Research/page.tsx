...
export default function ResearchPage() {
    ...
    useEffect(() => {
        setVisibleCount(12);
    }, [search, selectedDept, selectedFaculty, publicationType, sortBy, yearRange, citationRange, impactLevel]);

    const departments = useMemo(() => {
        return Array.from(new Set(data.map((item) => item.department))).filter(d => d && d !== "Other").sort();
    }, [data]);
    ...
    const filteredData = useMemo(() => {
        let result = data.filter((item) => {
            ...
            const matchesFaculty = selectedFaculty === "All" || item.authors === selectedFaculty;
            const matchesType = publicationType === "All" || item.category === publicationType;
            ...
            else if (citationRange === "50-100") matchesCitationRange = cites > 50 && cites <= 100;
            ...
            return matchesSearch && matchesDept && matchesFaculty && matchesType && matchesYearRange && matchesCitationRange && matchesImpactLevel;
        });
        ...
        return result;
    }, [data, search, selectedDept, selectedFaculty, sortBy, yearRange, citationRange, impactLevel]);
    ...
    const stats = useMemo(() => {
        return {
            ...
            faculty: Array.from(new Set(filteredData.filter(d => d.authorType === "Facutly" || d.authorType === "Faculty").map(d => d.authors))).length,
            researchers: Array.from(new Set(filteredData.map(d => d.authors))).length
        };
    }, [filteredData]);
                ...
                <div className="container relative z-10 mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="mb-16 grid grid-cols-1 gap-6 md:grid-cols-4">
                        {[
                            ...
                            { label: "Active Researchers", value: stats.researchers, icon: Users, color: "text-purple-600", bg: "bg-purple-50" }
                        ].map((stat, i) => (
                            <motion.div
                                key={stat.label}
                                initial={{ opacity: 0, y: 20 }}
                            ...
                            >
                                <div className={`mb-4 inline-flex rounded-2xl ${stat.bg} ${stat.color} p-4 group-hover:scale-110 transition-transform`}>
                                    <stat.icon size={28} />
                                </div>
                        ...
                        <>
                                ...
                                <AnimatePresence mode="popLayout">
                                    {paginatedData.length > 0 ? (
                                        ...
                                        <div className="col-span-full py-32 text-center">
                                            ...
                                            <button
                                                onClick={() => {
                                                    setSearch(""); setSelectedDept("All"); setSelectedFaculty("All");
                                                    setPublicationType("All"); setYearRange("All");
                            ...
                            {visibleCount < filteredData.length && (
                                <div className="mt-20 flex justify-center">
                                    ...
                                    >
                                        <span className="relative z-10">Expand Intelligence Layer</span>
