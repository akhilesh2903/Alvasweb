...
import { Float, Sphere, MeshDistortMaterial, Torus, Environment, Icosahedron, Octahedron, Dodecahedron } from "@react-three/drei";
...
function Particles({ count = 3000 }) {
    ...
    return (
        <points ref={mesh}>
            ...
            <pointsMaterial size={0.02} vertexColors transparent opacity={0.25} sizeAttenuation />
        </points>
    );
}
        ...
        <group ref={groupRef} position={[5.5, -1, -5]} scale={0.5}>
            {spherePositions.map(([x, y, z, color], i) => (
                <mesh key={`dna2-${i}`} position={[x, y, z]}>
                    ...
                    <meshStandardMaterial color={color} emissive={color} emissiveIntensity={0.1} transparent opacity={0.55} />
                </mesh>
            ))}
            {connectorData.map(([x1, y1, z1, x2, y2, z2], i) => {
                const mid: [number, number, number] = [(x1 + x2) / 2, (y1 + y2) / 2, (z1 + z2) / 2];
                const length = Math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2 + (z2 - z1) ** 2);
                return (
                    <mesh key={`conn2-${i}`} position={mid} rotation={[0, 0, Math.PI / 2]}>
...
function AtomModel({ position, scale = 0.6 }: { position: [number, number, number]; scale?: number }) {
    const groupRef = useRef<THREE.Group>(null!);
    ...
    return (
        <group ref={groupRef} position={position} scale={scale}>
            <Sphere args={[0.15, 16, 16]}>
                <MeshDistortMaterial color="#6366f1" speed={4} distort={0.3} opacity={0.7} transparent emissive="#6366f1" emissiveIntensity={0.2} />
            </Sphere>
            <Torus args={[0.6, 0.012, 8, 64]} rotation={[0, 0, 0]}>
                <meshStandardMaterial color="#3b82f6" transparent opacity={0.35} />
            </Torus>
            <Torus args={[0.6, 0.012, 8, 64]} rotation={[Math.PI / 3, Math.PI / 6, 0]}>
                <meshStandardMaterial color="#06b6d4" transparent opacity={0.35} />
            </Torus>
            <Torus args={[0.6, 0.012, 8, 64]} rotation={[-Math.PI / 3, -Math.PI / 6, 0]}>
...
function ElectronBead({ orbitIndex }: { orbitIndex: number }) {
    ...
    useFrame((state) => {
        ...
        const t = state.clock.getElapsedTime() * speed + orbitIndex * (Math.PI * 2) / 3;
        const r = 0.6;
...
function MoleculeCluster({ position }: { position: [number, number, number] }) {
    ...
    const nodes: [number, number, number][] = [
        [0, 0, 0], [0.5, 0.3, 0.1], [-0.4, 0.4, -0.2],
        [0.2, -0.5, 0.3], [-0.3, -0.3, -0.4], [0.4, -0.2, -0.3],
    ];

    const bonds: [number, number][] = [
        [0, 1], [0, 2], [0, 3], [0, 4], [1, 2], [3, 5], [4, 5],
    ];
    ...
    return (
        <Float speed={1.5} rotationIntensity={0.5} floatIntensity={1}>
            <group ref={groupRef} position={position} scale={0.8}>
                {nodes.map(([x, y, z], i) => (
                    <Sphere key={`mol-node-${i}`} args={[i === 0 ? 0.12 : 0.08, 16, 16]} position={[x, y, z]}>
                ...
                {bonds.map(([a, b], i) => {
                    ...
                    return (
                        <mesh key={`bond-${i}`} position={[mid.x, mid.y, mid.z]} quaternion={quaternion}>
                            <cylinderGeometry args={[0.012, 0.012, length, 6]} />
...
function FloatingResearchShapes() {
    return (
        <>
            ...
            <Float speed={2} rotationIntensity={1.5} floatIntensity={1.5} position={[4.5, 2.5, -4]}>
            ...
            {/* Wireframe Flask */}
            <Float speed={1.8} rotationIntensity={0.8} floatIntensity={2} position={[-3.5, -3, -2]}>
                <group scale={0.45}>
            ...
            <Float speed={3} rotationIntensity={2} floatIntensity={1} position={[3, -2, -5]}>
                <Sphere args={[0.3, 32, 32]}>
                    <MeshDistortMaterial color="#8b5cf6" speed={4} distort={0.5} opacity={0.25} transparent />
                </Sphere>
            </Float>

            {/* Pulsing green sphere */}
            <Float speed={4} rotationIntensity={0.5} floatIntensity={3} position={[5.5, 3, -6]}>
                <Sphere args={[0.18, 32, 32]}>
                    <MeshDistortMaterial color="#10b981" speed={5} distort={0.6} opacity={0.2} transparent />
                </Sphere>
            </Float>

            {/* Hexagonal ring */}
            <Float speed={1.5} rotationIntensity={1} floatIntensity={1.5} position={[-5.5, 2.5, -3]}>
            ...
            <Float speed={2.5} rotationIntensity={2} floatIntensity={1} position={[-2, 3.5, -4]}>
            ...
            {/* Octahedron */}
            <Float speed={1.8} rotationIntensity={1.5} floatIntensity={2} position={[2, -3.5, -3]}>
            ...
            {/* Dodecahedron */}
            <Float speed={2} rotationIntensity={1} floatIntensity={1.5} position={[6, 0, -5]}>
                <Dodecahedron args={[0.2, 0]}>
                    <meshStandardMaterial color="#f43f5e" transparent opacity={0.18} wireframe />
                </Dodecahedron>
            </Float>

            {/* Additional small floating spheres */}
            <Float speed={3.5} rotationIntensity={0.3} floatIntensity={2.5} position={[-4, -1, -6]}>
                <Sphere args={[0.1, 16, 16]}>
                    <meshStandardMaterial color="#f59e0b" transparent opacity={0.3} emissive="#f59e0b" emissiveIntensity={0.1} />
                </Sphere>
            </Float>

            <Float speed={2.8} rotationIntensity={0.5} floatIntensity={2} position={[1, 4, -7]}>
                <Sphere args={[0.12, 16, 16]}>
                    <meshStandardMaterial color="#06b6d4" transparent opacity={0.25} emissive="#06b6d4" emissiveIntensity={0.1} />
                </Sphere>
            </Float>
        </>
    );
    ...
    return (
        <div className="fixed inset-0 -z-10 bg-gradient-to-br from-slate-50 via-white to-blue-50">
            <Canvas camera={{ position: [0, 0, 8], fov: 60 }}>
                ...
                <directionalLight position={[5, 5, 5]} intensity={0.5} color="#bfdbfe" />
                <Particles />
                <DNAHelix />
                <DNAHelixRight />
