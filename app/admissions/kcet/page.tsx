...
export default function AdmissionThroughKCETPage() {
  ...
  const steps = [
    {
      step: "01",
      title: "Register for KCET",
      desc: "Candidates must appear for the Karnataka Common Entrance Test (KCET) conducted by the Karnataka Examinations Authority (KEA). Register online at the official KEA website.",
      icon: "fas fa-user-plus",
    },
    {
      step: "02",
      title: "Obtain KCET Rank",
      desc: "After the examination, collect your KCET rank card from the KEA portal. The rank will determine your eligibility for seat allotment in various colleges.",
      icon: "fas fa-list-ol",
    },
    {
      step: "03",
      title: "Participate in Online Counseling",
      desc: "Register for KEA online counseling. Submit your college and branch preferences in order of priority. Participate in document verification at the designated Help Line Centre (HLC).",
      icon: "fas fa-laptop",
    },
    {
      step: "04",
      title: "Seat Allotment",
      desc: "Seats are allotted based on KCET rank, category, and preferences submitted. Check the allotment result on the KEA portal and download your Seat Allotment Order.",
      icon: "fas fa-check-circle",
    },
    {
      step: "05",
      title: "Report to AIET",
      desc: "Report to Alva's Institute of Engineering & Technology with all original documents and the allotment order within the stipulated time. Pay the required fees to confirm admission.",
      icon: "fas fa-university",
    },
  ];

  const eligibility = [
    "Passed 2nd PUC / 12th Standard with English as one of the Languages",
    "Minimum 45% aggregate in Physics and Mathematics as compulsory subjects along with Chemistry / Bio-Technology / Biology / Electronics / Computer Science",
    "For Karnataka SC, ST, and OBC candidates — minimum aggregate is 40%",
    "Must have appeared and qualified in KCET conducted by KEA",
    "Karnataka domicile or eligible NRI / PIO candidates as per KEA norms",
  ];
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 font-sans cursor-default overflow-x-hidden">
      ...
      <section className="relative h-[48vh] min-h-[280px] w-full mt-[80px] overflow-hidden">
        ...
        <div className="absolute -bottom-16 -left-10 w-[300px] h-[300px] rounded-full pointer-events-none" style={{ background: "radial-gradient(circle,rgba(59,130,246,0.2) 0%,transparent 65%)" }} />
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
          ...
          <h1 className="text-4xl md:text-6xl font-black text-white mb-4 drop-shadow-2xl font-serif italic uppercase tracking-wide">
            Admission Through <span className="text-yellow-400">KCET</span>
          </h1>
          <div className="w-24 h-1.5 bg-yellow-400 mx-auto rounded-full mb-4" />
          <p className="text-white/70 max-w-lg font-semibold text-sm tracking-wide">Karnataka Common Entrance Test — KEA Counseling Process</p>
        </div>
      </section>
        ...
        <main className="relative z-10 container mx-auto px-6 py-14 max-w-5xl space-y-12">
          ...
          <section className="bg-white/80 backdrop-blur-md rounded-3xl p-8 md:p-10 shadow-xl border border-slate-100">
            <div className="flex items-center gap-4 mb-5">
              <span className="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center"><i className="fas fa-info-circle text-blue-700 text-lg" /></span>
              <h2 className="text-blue-900 font-black text-xl">About KCET Admission</h2>
            </div>
            <p className="text-slate-700 leading-relaxed">
              The Karnataka Common Entrance Test (KCET) is conducted by the <strong className="text-blue-900">Karnataka Examinations Authority (KEA)</strong> for admission to Engineering, Architecture, Pharmacy, and other professional courses in Karnataka. Admissions to AIET through KCET are governed by the rules and regulations laid down by KEA and the Government of Karnataka.
            </p>
          </section>
            ...
            <div className="space-y-5">
              {steps.map((s) => (
                <div key={s.step} className="bg-white/80 backdrop-blur-sm rounded-2xl p-6 shadow-md border border-slate-100 flex gap-5 hover:border-yellow-200 transition-colors">
                  <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-blue-900 text-white flex items-center justify-center font-black text-lg">{s.step}</div>
                  <div>
          ...
          {/* Documents */}
          <section className="bg-white/80 backdrop-blur-md rounded-3xl shadow-xl border border-slate-100 overflow-hidden">
            <div className="flex items-center gap-3 px-7 py-4 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-white">
              <span className="w-9 h-9 rounded-xl bg-blue-100 flex items-center justify-center"><i className="fas fa-file-alt text-blue-700 text-base" /></span>
              <h2 className="text-blue-900 font-black text-lg">Documents Required at the Time of Admission</h2>
            </div>
            <ul className="px-7 py-6 space-y-3">
              {documents.map((doc, i) => (
                <li key={i} className="flex items-start gap-3 group">
                  <span className="mt-0.5 flex-shrink-0 w-6 h-6 rounded-full bg-blue-50 text-blue-700 border border-blue-200 flex items-center justify-center text-xs font-black">{i + 1}</span>
